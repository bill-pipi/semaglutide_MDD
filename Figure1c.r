...
workdir <- "."
datafile <- file.path(workdir,"Figure1c.csv")
...
tabletext <- cbind(c("Population",data$blah),
                    c("Semaglutide\ncohort\n",data$case_group),
                    c("Non-GLP-1RAs\n anti-obesity medications cohort\n",data$control_group),
                    c("        HR (95% CI)",data$CI)
)

pdf(file.path(workdir,"Figure1c.pdf"),  onefile=FALSE, width=10, height=6)
...
forestplot(labeltext=tabletext, 
           ...
           title ="Receipt of psychotherapy during a 12-month follow-up\nin patients with obesity and MDD \n(comparison between propensity-score matched cohorts)", 
                      
           #xticks=log(c(0.3,0.4,0.5,0.6,0.7,0.8,0.9,1,2,3)),
          
           xticks=log(c(0.5,0.6,0.7,0.8,0.9,1,2)),
           xlog = TRUE,
